...
// Column names 
$fields = array('END WEIGHT DATE', 'ORDER NO', 'NUMBER OF BIRDS', 'NET WEIGHT', 'HOUSE NUMBER'); 
...
$query = $db->query("select * FROM weighing WHERE deleted = '0' AND start_time IS NOT NULL AND end_time IS NOT NULL".$searchQuery." ORDER BY farm_id DESC");
...
if($query->num_rows > 0){ 
    ...
    while($row = $query->fetch_assoc()){ 
        ...
        for($i=0; $i<count($weight_data); $i++){
            if(!in_array($weight_data[$i]['houseNumber'], $groupCheck)){
                $groupList[] = array(
                ...
                array_push($groupCheck, $weight_data[$i]['houseNumber']);
            }
            
            $key = array_search($weight_data[$i]['houseNumber'], $groupCheck);
        ...
        for($j=0; $j<count($groupList); $j++){
            ...
            //array_push($farmList, array($row['end_time'], $row['po_no'], $groupList[$j]['totalBird'], $totalNet, $groupList[$j]['houseNumber']));
            $dateTime = DateTime::createFromFormat('Y-m-d H:i:s', $row['end_time']);
            $formattedDate = $dateTime->format('m/d/Y');
            $lineData = array($formattedDate, $row['po_no'], $groupList[$j]['totalBird'], $totalNet, $groupList[$j]['houseNumber']);
